	...
	init(){
        return new Promise(function (resolve,reject) {
            ...
            self.promiseRun(sql).then((result)=>{
                ...
                if (result) {
                    let sql6 = "CREATE TABLE IF NOT EXISTS tb_erc20_pi_transaction(id integer PRIMARY KEY, chainId text,funCode text,preimage text,ethContractId text,piContractId text,hash text,fromaddress text,toaddress text,value text,status integer,createTime text);";
